...
MovieLoader::MovieLoader(std::string path) {
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            // Znajdź pozycje przecinków
            size_t pos1 = line.find(',');
            if (pos1 == std::string::npos) continue; // Skip lines without commas
            size_t pos2 = line.find(',', pos1 + 1);
            if (pos2 == std::string::npos) continue; // Skip lines without second comma
            size_t pos3 = line.find(',');
            if (pos3 == std::string::npos) continue; // Skip lines without third comma
            size_t pos4 = line.find(',',pos2 +1);
            if (pos4 != std::string::npos) continue; // Skip lines without third comma
